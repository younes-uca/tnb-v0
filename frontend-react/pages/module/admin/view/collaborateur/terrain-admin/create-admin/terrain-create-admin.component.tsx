...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const [items, setItems] = useState<TerrainDto[]>([list]);
    ...
    const saveItem = async () => {
        ...
        let _item = {...item};
        if (!_item.id) {
            const response = await TerrainService.save(_item);
			_item.id = response.data.id;
            _items.push(_item);
            add(_item);
            MessageService.showToast(showToast, { severity: 'success', summary: 'Successful', detail: 'Terrain Created', life: 3000 });
        }
        setItems(_items);
        onClose();
        setItem(emptyItem);
        ...
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Terrain">
                <div className="formgrid grid">
                    ...
                    <InputText id="ref" value={item.ref} onChange={(e) => onInputTextChange(e, 'ref')} required autoFocus className={classNames({'p-invalid': submitted && !item.ref})} />
