...
import Edit from '/pages/module/admin/view/collaborateur/redevable-admin/edit-admin/redevable-edit-admin.component';
import Create from '/pages/module/admin/view/collaborateur/redevable-admin/create-admin/redevable-create-admin.component';
import View from '/pages/module/admin/view/collaborateur/redevable-admin/view-admin/redevable-view-admin.component';

const List = () => {

    const emptyItem = new RedevableDto();
    const [items, setItems] = useState<RedevableDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<RedevableDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<RedevableDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<RedevableDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new RedevableCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<RedevableDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new RedevableCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await RedevableService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: RedevableDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: RedevableDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: RedevableDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: RedevableDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await RedevableService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'Redevable Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await RedevableService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'Redevables Deleted', life: 3000 });
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: RedevableDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage redevables</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <Card>
                    <div className="search-container">
                        <div className="grid">
                                        ...
                                        <InputText id="1" value={criteria.nom} onChange={(e) => setCriteria({ ...criteria, nom: e.target.value })} />
                                        <label htmlFor="1">Nom</label>
                                        </span>
                                        <span className="p-float-label mr-3 align-search-items mt-4">
                                        <InputText id="2" value={criteria.prenom} onChange={(e) => setCriteria({ ...criteria, prenom: e.target.value })} />
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as RedevableDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete redevable <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected redevables?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
