...
import Edit from '/pages/module/admin/view/commun/categorie-terrain-admin/edit-admin/categorie-terrain-edit-admin.component';
import Create from '/pages/module/admin/view/commun/categorie-terrain-admin/create-admin/categorie-terrain-create-admin.component';
import View from '/pages/module/admin/view/commun/categorie-terrain-admin/view-admin/categorie-terrain-view-admin.component';

const List = () => {

    const emptyItem = new CategorieTerrainDto();
    const [items, setItems] = useState<CategorieTerrainDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<CategorieTerrainDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<CategorieTerrainDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<CategorieTerrainDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new CategorieTerrainCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<CategorieTerrainDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new CategorieTerrainCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await CategorieTerrainService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: CategorieTerrainDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: CategorieTerrainDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: CategorieTerrainDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: CategorieTerrainDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await CategorieTerrainService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'CategorieTerrain Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await CategorieTerrainService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'CategorieTerrains Deleted', life: 3000 });
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: CategorieTerrainDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage categorie terrains</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as CategorieTerrainDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete categorie terrain <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected categorie terrains?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
